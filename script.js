...
class CoupleTodoApp {
    ...
    addTask() {
        ...
        if (this.partnerInfo) {
            if (
                (this.partnerInfo.type === 'partner1' && partner === 'partner2') ||
                (this.partnerInfo.type === 'partner2' && partner === 'partner1')
            ) {
                this.showRestrictionModal(partner);
                return;
            }
        }
    ...
    showRestrictionModal(partner) {
        const modal = document.getElementById('restrictionModal');
        const message = document.getElementById('restrictionMessage');
        let role = '';
        if (partner === 'partner1') role = 'Partner 1';
        else if (partner === 'partner2') role = 'Partner 2';
        else role = 'this role';
        message.textContent = `You are not allowed to add tasks for this role. This task is restricted to ${role}.`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Restriction modal close button
